...
namespace GateScheduler.Solution
{
    public struct TimeRange
    {
        ...
        public static readonly TimeSpan EmptyTime = TimeSpan.FromHours(0);
        ...
        {
            TimeSpan start = time > Start ? EmptyTime : Start - time;
            return new TimeRange(start, End);
        }

        public TimeRange ReduceEnd(TimeSpan time)
        {
            TimeSpan end = OneDay - End < time ? OneDay : End + time;
            return new TimeRange(Start, end);
        }
